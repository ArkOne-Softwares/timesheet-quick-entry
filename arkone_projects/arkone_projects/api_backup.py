...
def update_task(task_name, data):
    """
    Update task details (for edit form)
    """
    try:
        ...
        if not frappe.has_permission('Task', 'write', task_name):
            frappe.throw(_("You do not have permission to edit this task"))
        ...
        if 'exp_start_date' in data:
            task.exp_start_date = data['exp_start_date']
        if 'exp_end_date' in data:
        ...
        # Auto-update progress based on status
        if data.get('status') == 'Completed' and task.progress < 100:
            task.progress = 100
        elif data.get('status') == 'Working' and task.progress == 0:
            task.progress = 10
